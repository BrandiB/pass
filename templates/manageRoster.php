...
$nextRoster = date("F j, Y", strtotime('Monday next week'));
$whichDay[1] = date("Y-m-d", strtotime('Monday next week'));
$tableDay[1] = date("D, M j", strtotime('Monday next week')); //tableDay is formatted for table header
$tableDay[2] = date("D, M j", strtotime('Tuesday next week')); 
$whichDay[2] = date("Y-m-d", strtotime('Tuesday next week')); //whichDay is formatted for database
$tableDay[3] = date("D, M j", strtotime('Wednesday next week'));
$whichDay[3] = date("Y-m-d", strtotime('Wednesday next week'));
$tableDay[4] = date("D, M j", strtotime('Thursday next week'));
$whichDay[4] = date("Y-m-d", strtotime('Thursday next week'));
$tableDay[5] = date("D, M j", strtotime('Friday next week'));
...
echo "<script language='JavaScript'>
	...
	function overflowStudent(whichStudent, theDay) {
		$.ajax({
			...
			data: { keyid: '".$keyid."', whichStudent: whichStudent, theDay: theDay, whichWeek: '2' },
			}).done(
	...
	function cancelRequest(whichRequest) {
		$.ajax({ 
			...
			data: { whichRequest: whichRequest, keyid: '".$keyid."', whichWeek: '2' },
			}).done(
			function(output) { $('#formdisplay').html(output).show();  });
	}
...
if (mysql_num_rows($result5) != 0) { // there are requests
	while ($row5=mysql_fetch_array($result5)) {
		...
		if ($lastStudent != $currentStudent) { // skip if repeat
			...
			while ($theDay < 6) {
				$query6 = "select requestmatrix.categoryID, requestmatrix.requestID, category.categoryName from pass.requestmatrix, pass.category where requestmatrix.studentID=".$currentStudentID." and requestmatrix.dateRequested='".$whichDay[$theDay]."' and category.categoryID=requestmatrix.categoryID";
